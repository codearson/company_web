      ...
      <div className="container">
        ...
        <div className="controls">
          {
            rtl ? (
              ...
              projectsData.types.map((type, i) => (
                <button 
                  key={i} 
                  type="button" 
                  className={`control ${activeTab === type.tab ? 'active' : ''}`}
        ...
        <section className="portfolio style-1">
            ...
            <div className="row mix-container">
              {
                rtl ? (
                  projectsData.projects.map((project, i) => (
                    <div className={`col-lg-4 mix ${project.filter}`} key={i}>
                      <div className={`portfolio-card ${i !== projectsData.projects.length - 1 ? 'mb-50':''}`}>
                        ...
                        <div className="info">
                          <h5>
                            <Link to={rtl ? "/rtl-page-single-project" : "/page-single-project-5"}>
                              { project.title }
                            </Link>
                          </h5>
                          ...
                          <div className="tags">
                            {
                              project.tags.map((tag, i) => (
                                <button key={i} className="btn btn-link p-0 me-1 text-decoration-none">{ tag }</button>
                              ))
                            }
                          </div>
                        </div>
                  ...
                  projectsData.tabs.find(tab => tab.id === activeTab)?.projects.map((project, i) => (
                    <div className="col-lg-4 mix" key={i}>
                      <div className={`portfolio-card ${i !== projectsData.tabs.find(tab => tab.id === activeTab).projects.length - 1 ? 'mb-50':''}`}>
                        <div className="img">
                          <img src={project.image} alt="" />
                        </div>
                        <div className="info">
                          ...
                          <small className="d-block color-main text-uppercase">{ project.description }</small>
                        </div>
                      </div>
                    </div>
                  ))
          ...
          <div className="text-center">
            <Link to="/portfolio" className={`btn rounded-pill fw-bold ${style === '5' ? 'mt-30 sm-butn hover-blue2 blue5-3Dbutn':'me-4 text-white bg-blue4'}`}>
